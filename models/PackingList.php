...
class PackingList
{
    ...
    {
        $Query = 'SELECT ';
        $Query .= 'doctes.id as id_testa, doctes.numerodoc, doctes.datadoc, doctes.codicecf, ';
    ...
    public function getField($field)
    {
        ...
        if (strcasecmp($this->getFieldTypes($field), 'boolean') == 0) {
            return ($this->Recordset[$this->_currentRow][$field] == 'True') ? 1 : 0; //'True' : 'False';
        } elseif (strcasecmp($this->getFieldTypes($field), 'float') == 0) {
