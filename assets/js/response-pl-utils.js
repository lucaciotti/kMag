...
function decodeArt(codice, obj) {
    ...
    if(val != "") {
		...
		var tbox = htmlEntities(String(document.getElementById('codart').innerHTML));
		var str = String(oArt.codice);
		//console.log(tbox +" "+ str+" "+ tbox.length+" "+ str.length);
...
var checkQta = function(cCodice, obj) {
	...
	if (oldFatt != 0){
		...
		document.getElementById('newFatt').value = newFatt;
